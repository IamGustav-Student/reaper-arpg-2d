...
- [x] **Hito 7:** `RuneData` (Resource) + 3 Runas de Forma (Tajo Giro, Guadaña Espectral, Nova) + 2 de Modificador (Sed de Sangre, Ceniza de Sombra) + 2 de Trigger (Al Esquivar, Al Asestar Crítico) reales (`resources/runes/`) + `RuneSpellBuilder.compose()` que duplica un `AttackData` base y le suma los deltas de las runas (nunca muta el original). `AttackData` extendido con `hitbox_radius_multiplier`/`lifesteal_percent`/`dot_*`, consumidos por `Hitbox` (radio dinámico, recalculado siempre desde el radio base del `.tscn`) y `Hurtbox` (robo de vida, daño por tiempo). Verificado con escena de debug (removida): composición, daño, robo de vida y radio, todos con los números exactos calculados a mano.
- [x] **Hito 8:** Altar de Almas (UI, tecla **L**) — `RuneAltarManager` (autoload) desbloquea runas gastando Esencia de Alma (`SoulHarvestManager.consume_charge`, 30 por runa) y las equipa en el Slot Activo o de Movilidad (hasta 3 runas por slot). El ataque real del Player (`_start_attack`) ahora compone su `Hitbox.attack_data` desde `RuneAltarManager.get_composed_attack_data()` — sin runas equipadas se comporta exactamente igual que antes. Los Triggers todavía no disparan nada automático (son metadata; el disparo por esquivar/crítico queda pendiente, y "crítico" ni siquiera existe todavía como mecánica real — `StatSystem.crit_chance` se calcula pero nunca se tira/usa en el daño).
- [x] **Hito 9:** Persistencia de estado de Chunks y enemigos derrotados (el "Hito 7" original antes de esta actualización de roadmap) — `GameManager` (autoload) guarda qué índice de enemigo de qué `Vector2i` de chunk ya murió y qué Boss Chunks ya se derrotaron, deliberadamente en el autoload y no en `ChunkManager` para sobrevivir tanto a descarga/recarga de chunk como a `reload_current_scene()` en la muerte del jugador. `ChunkManager._generate_forest_chunk()` consulta `GameManager.is_enemy_killed()`/`is_boss_defeated()` antes de spawnear (el índice de spawn es determinístico dada la semilla del chunk) y cada enemigo/boss conecta su `health_system.died` a `GameManager.mark_enemy_killed()`/`mark_boss_defeated()`. Verificado con escena de debug (removida): tras matar 1 de 2 enemigos de un chunk, descargarlo (jugador lejos) y recargarlo (jugador vuelve), sigue vivo solo 1 — el muerto no reaparece.
...
- [x] **Hito 11:** `TelegraphArea` + Boss Chunks determinísticos (`ChunkManager._is_boss_chunk`, ~1/7 de los chunks a distancia ≥5) + primer World Boss **"Treant Ancestral"** (`BossTreant extends Enemy`, 6x escala, 900 HP, Fase 1 solo cuerpo a cuerpo, transición invulnerable 1.5s con Hit Flash sostenido, Fase 2 suma "Pisotón Sísmico" telegrafiado). Recompensa: un Orbe de Alma que llena el Medidor de Almas al 100% de un golpe (100 de Esencia — corregido de un valor de 200 que originalmente excedía inútilmente el tope del medidor). Verificado con escena de debug (removida): daño de golpe y de AoE exactos, transición de fase, y tasa real de Boss Chunks (~15%, esperado ~14%).
- [x] **Hito 12:** Segundo tipo de enemigo con ataque a distancia — `RangedEnemy extends Enemy` (reemplaza la IA de persecución por mantener `preferred_range` y disparar) + `Projectile extends Hitbox` (un Hitbox con movimiento en línea recta, se destruye al primer impacto o por `lifetime`) — primer uso de un proyectil real en el juego, reusando por completo la detección de golpes de Hitbox/Hurtbox. Sprite reusa el Mole existente con tinte verde (sin arte nuevo). Verificado con escena de debug (removida): retirada al acercarse el jugador, disparo al llegar al rango preferido, e impacto real sobre el Player.
- [x] **Hito 13:** Sistema de Habilidades real (cierra los huecos de metadata-sin-efecto que dejó el Hito 7-8, inspirado en *Echoes of Mystralia* — `docs/ECHOES_OF_MYSTRALIA_ANALYSIS.md`):
  * **Crítico real:** `StatSystem.crit_multiplier` (1.5x, sin stat que lo escale todavía) + `Hurtbox.receive_hit()` tira `crit_chance` y devuelve si fue crítico. `Hitbox` reenvía eso como nueva señal `critical_hit_landed`.
  * **Runas de Desencadenante reales:** "Al Esquivar" (Slot Movilidad) dispara lo crafteado ahí al iniciar el Dash; "Al Asestar Crítico" (Slot Activo) relanza lo crafteado ahí cuando el swing manual conecta un crítico. Ambos disparos usan un nodo suelto (`SpellBurst`, nunca el `Hitbox` propio del Player) para no pisar su máquina de estados de ataque/dash — por eso tampoco pueden recursar. El relanzamiento por crítico llega desde `Hitbox._on_area_entered` (corre dentro del flush de queries del motor de físicas), así que se difiere con `call_deferred()` — instanciar un nodo con colisión ahí directamente rompía con "Can't change this state while flushing queries".
  * **Nueva Runa de Forma "Proyectil Espectral"** (`RuneData.makes_projectile`): si está equipada, el ataque compuesto se lanza como `SpectralProjectile extends Projectile` (dibujado con `_draw()`, sin arte nuevo, mismo criterio que `SoulOrb`) en vez de activar el Hitbox cuerpo a cuerpo — funciona tanto en el swing manual como en cualquier Runa de Desencadenante que lo dispare.
  * **Fix de paso:** `RangedEnemy` nunca seteaba el atacante real de su `Projectile` (`get_owner()` da null en un nodo suelto), así que su daño ignoraba silenciosamente `power_multiplier` de Tier/Élite — ahora usa `Hitbox.source_override`.
  * Altar de Almas (UI) actualizado a 8 Runas. Verificado con escena de debug (removida): crítico forzado (45 = 30×1.5) vs. sin crítico (30), Dash con "Al Esquivar" (30), swing+relanzado con "Al Asestar Crítico" (90 = 45+45), y Proyectil Espectral golpeando un objetivo fuera de alcance melee (30) — los 5 con los números exactos calculados a mano.
