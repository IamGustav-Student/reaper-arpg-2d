...
| `art/environment/tiny_rpg_forest/` | [Tiny RPG Forest by Luis Zuno (ansimuz)](https://opengameart.org/content/tiny-rpg-forest) — OpenGameArt | CC0 / dominio público (crédito no obligatorio) | Árboles, arbustos, rocas, tronco, cartel (bosque, reusados proceduralmente por `ChunkManager`) + pared de piedra/puerta/entrada de cueva + piso de tierra tileable (`ruins/`) para la ruina y los chunks de bosque + enemigos **Mole** y **Treant** (`enemies/`, `scenes/enemies/mole.tscn`/`treant.tscn` con Hurtbox+HealthSystem+IA reales) |
| `art/characters/hero/` | Mismo pack (Tiny RPG Forest) — héroe arquero animado | CC0 | Sprites de Idle/Walk(6 frames)/Attack-con-arma(3 frames) x Front/Back/Side, usados en `scenes/player/hero_sprite_frames.tres` como placeholder animado del Player (side se espeja con flip_h para izquierda) |
